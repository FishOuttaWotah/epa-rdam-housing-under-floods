...
# --------
# COMMENT: not used? Also redundant?
# --------
def flood_handler_prob(shock_prob, n_floods):
    """TODO: write description.

    Args:
        shock_prob      : Probability of flooding
        n_floods        : Number of floods occurred so far

    Returns:
        flood           : Boolean; True if flood occurs
        n_floods        : Number of floods occurred so far
    """
    flood = False
    if bernoulli.rvs(shock_prob) == 1:
        flood = True
        n_floods += 1

    return flood, n_floods
